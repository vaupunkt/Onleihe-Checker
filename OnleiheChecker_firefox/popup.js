document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (languageSelect) {
        ...
        languageSelect.addEventListener('change', async (e) => {
            ...
            await storage.local.set({ selectedLanguage: selectedLanguage });
    ...
    try {
        const response = await fetch(runtime.getURL('libraries.json'));
        ...
        if (allLibraries && allLibraries.length > 0) {
            ...
            storage.local.get(['selectedOnleiheLibraryURL', 'selectedOnleiheLibraryName'], (result) => {
